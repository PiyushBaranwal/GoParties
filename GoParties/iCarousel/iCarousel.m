...
- (void)step
{
  ...
  //  _scrollOffset=0.005312;
    
    if (_scrolling && !_dragging)
    {
        NSTimeInterval time = MIN(1.0, (currentTime - _startTime) / _scrollDuration);//1.0
    ...
    else if (_autoscroll && !_dragging)
    {
        ...
        self.scrollOffset = [self clampedOffset:_scrollOffset - delta * _autoscroll];//_scrollOffset - delta * _autoscroll //1.00
        //NSLog(@"_scrollOffset=%f",_scrollOffset);
        //NSLog(@"delta=%f",delta);
       // NSLog(@"_autoscroll=%f",_autoscroll);
        //NSLog(@"_scrollOffset - delta * _autoscroll=%f",_scrollOffset - delta * _autoscroll);
    }
